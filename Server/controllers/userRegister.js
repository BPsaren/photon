...
import  {oneMinuteOtpExpire,threeMinuteOtpExpire}from '../helpers/otpvalidate.js'
//for delete photo
  ...
  try {
    ...
    if (userData) {
      if (userData.isVerified) {
        return res.render('mail-verification', { message: 'Your email is already verified' });
      }

      await User.findByIdAndUpdate(req.query.id, { $set: { isVerified: true } });
...
const generateAcessToken=async(user)=>{
  const token = await jwt.sign(user,process.env.ACCESS_TOKEN_SECRECT,{expiresIn:'2h'});
  return token;
}


const generateRefreshToken=async(user)=>{
  const token = await jwt.sign(user,process.env.ACCESS_TOKEN_SECRECT,{expiresIn:'4h'});
  return token;
}
    ...
    try {
      ...
      if (req.file !== undefined) {
        ...
        if (oldUser && oldUser.image) {
          const oldFilePath = path.join(process.cwd(), 'public', oldUser.image); // Corrected path
...
const optVerification = async (req, res) => {
    try {
        ...
        if (!otpData) {
            return res.status(400).json({ success: false, message: 'Invalid OTP or user ID' });
        }
        ...
        return res.status(200).json({ success: true, message: 'Your email has been successfully verified' });
